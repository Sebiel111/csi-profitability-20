...
st.markdown("""<style>
  ...
  .stSlider label, .stSlider { font-size: 1.5rem !important; }
  .input-col {
    display: flex;
  ...
  .input-col .stTextInput > div > input {
    font-size: 22px !important;
...
# --- Labels and Locales ---
labels = {
    "English": {"lang":"en_US","language":"Language","title":"CSI Profitability Simulator",
                "csi_score":"CSI score (out of 1,000)","sample":"Sample size (Volvo Selekt sales)",
                ...
                "service_cust":"Service customers","repeat":"Repeat purchases",
                "total_profit":"Total profit","total":"Total","download":"Download CSV"},
    "Español": {"lang":"es_ES","language":"Idioma","title":"Simulador de rentabilidad CSI",
                "csi_score":"Puntuación CSI (de 1 000)","sample":"Volumen de ventas Volvo Selekt",
                ...
                "service_cust":"Clientes de servicio","repeat":"Recompras",
                "total_profit":"Beneficio total","total":"Total","download":"Descargar CSV"},
    "Português": {"lang":"pt_PT","language":"Idioma","title":"Simulador de rentabilidade CSI",
                  "csi_score":"Pontuação CSI (de 1 000)","sample":"Vendas Volvo Selekt",
                  ...
                  "service_cust":"Clientes de serviço","repeat":"Recompras",
                  "total_profit":"Lucro total","total":"Total","download":"Baixar CSV"},
    "Français": {"lang":"fr_FR","language":"Langue","title":"Simulateur de rentabilité CSI",
                 "csi_score":"Score CSI (sur 1 000)","sample":"Nombre de ventes Volvo Selekt",
                 ...
                 "service_cust":"Clients service","repeat":"Achats répétés",
                 "total_profit":"Profit total","total":"Total","download":"Télécharger CSV"},
    "Deutsch": {"lang":"de_DE","language":"Sprache","title":"CSI-Ertragsimulator",
                "csi_score":"CSI-Wert (von 1 000)","sample":"Volvo Selekt-Verkäufe",
                ...
                "service_cust":"Servicekunden","repeat":"Wiederholungskäufe",
                "total_profit":"Gesamtgewinn","total":"Gesamt","download":"CSV herunterladen"},
    "Italiano": {"lang":"it_IT","language":"Lingua","title":"Simulatore di redditività CSI",
                 "csi_score":"Punteggio CSI (su 1 000)","sample":"Vendite Volvo Selekt",
                 ...
                 "service_cust":"Clienti assistenza","repeat":"Riacquisti",
                 "total_profit":"Profitto totale","total":"Totale","download":"Scarica CSV"},
    "Svenska": {"lang":"sv_SE","language":"Språk","title":"CSI Lönsamhetssimulator",
                "csi_score":"CSI-poäng (av 1 000)","sample":"Volvo Selekt-försäljning",
                ...
                "service_cust":"Servicekunder","repeat":"Återköp",
...
def simulate(csi, count, s_profit, own, warr, v_profit):
    ...
    totals = {
        L["year"]: L["total"],
        L["service_cust"]: df[L["service_cust"]].sum(),
        L["repeat"]: df[L["repeat"]].sum(), 
        L["total_profit"]: df[L["total_profit"]].sum()
    }
...
for col, key, lbl in zip(cols, keys, labels_input):
    with col:
        ...
        # Use empty value by default
        val = st.text_input("", key=key)
        # Immediately format after typing
        if val:
            st.session_state[key] = format_number(parse_int(val), sep)
...
if st.button(L["run"]):
    ...
    for _, row in df.iterrows():
        ...
        for val in row.iloc[1:]:
            html += f"<td>{format_number(val, sep)}</td>"
        html += "</tr>"
    html += "</tbody></table>"
    st.subheader(L["results"])
    st.markdown(html, unsafe_allow_html=True)
    st.download_button(L["download"], df.to_csv(index=False).encode('utf-8'), "csi.csv", "text/csv")
